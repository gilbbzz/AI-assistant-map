...
// FIXED: Removed duplicate /best-route, moved all routes BEFORE module.exports,
//        increased max_tokens to prevent JSON truncation, added rate limiting.
...
// ── AI-specific rate limiter (ketat – AI mahal) ──────────────────────
...
async function generate(prompt, systemInstruction, isJson = false) {
  try {
    const response = await getGroq().chat.completions.create({
      ...
      temperature: 0.7,
      max_tokens: isJson ? 800 : 600,   // FIX: ditingkatkan agar JSON tidak terpotong
...
function safeParseJSON(str, fallback = {}) {
  try {
    ...
    // Coba ekstrak JSON dari dalam string (kadang AI menambahkan teks sebelum/setelah JSON)
...
- "navigateTo"   : nama tempat PERSIS seperti yang diucapkan user (jangan ubah/translate)
...
JSON: {"reply":"Mencari rumah sakit umum terdekat dari lokasi Anda...","navigateTo":"Rumah Sakit Umum","locationType":"hospital","locationCity":"${context.city || 'Indonesia'}","locationHint":""}

User: "Mau ke McDonalds"  
JSON: {"reply":"Oke, mencari McDonald's terdekat!","navigateTo":"McDonald's","locationType":"fast_food","locationCity":"${context.city || 'Indonesia'}","locationHint":""}

User: "Ke Stasiun Gambir dong"
JSON: {"reply":"Siap, membuat rute ke Stasiun Gambir!","navigateTo":"Stasiun Gambir","locationType":"station","locationCity":"Jakarta","locationHint":""}
...
- Jawaban reply: ramah, singkat (max 80 kata), bahasa Indonesia natural`;
  ...
  try {
    ...
    res.json({ success: true, data: { reply: parsed.reply || aiText, navigateTo: parsed.navigateTo || null } });
  } catch (err) {
    console.error('[Groq] chat error:', err.message);
    res.json({
      success: true,
      data: { reply: 'Maaf, layanan AI sedang sibuk. Coba lagi nanti.', navigateTo: null },
    });
  }
}));
...
// ============ REKOMENDASI RUTE TERBAIK (AI pilih tipe rute) ============
// FIX: Satu handler bersih — sebelumnya ada 2 handler duplikat SETELAH module.exports (bug kritis)

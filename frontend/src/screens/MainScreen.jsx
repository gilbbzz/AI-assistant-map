...
export default function MainScreen() {
  ...
  // State GPS error — true jika user menolak/tidak bisa akses GPS
  ...
  const handleAINavigate  = useCallback((place) => { setActivePanel(null); handleDestinationSelect(place); }, [handleDestinationSelect, setActivePanel]);
  const handleStartNav    = useCallback(() => { if (!activeRoute) return; setActivePanel(null); setIsNavigating(true); setNavigationRoute(activeRoute); toast.success('Navigasi dimulai!', { icon: '🧭' }); }, [activeRoute, setActivePanel]);
    ...
    <div className="screen" style={{ background: '#0D1117' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 100, padding: '10px 12px' }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div
            style={{ flex: 1, background: 'rgba(22,27,34,0.97)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 13, padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 9, cursor: 'pointer', boxShadow: '0 2px 12px rgba(0,0,0,0.3)' }}
          ...
          >
            ...
            {destination && (
              <button onClick={e => { e.stopPropagation(); handleClearRoute(); }} style={{ background: 'none', border: 'none', color: '#8B949E', fontSize: 18, padding: 0, lineHeight: 1 }}>×</button>
            )}
          </div>
          <button
            style={{ width: 42, height: 42, background: 'rgba(22,27,34,0.97)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 8px rgba(0,0,0,0.3)', position: 'relative' }}
        ...
        {/* Badge GPS, cuaca & traffic — semuanya dari data real */}
        <div style={{ marginTop: 7, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {cityName && (
            <span style={{ background: 'rgba(13,17,23,0.85)', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '3px 10px', fontSize: 11, color: '#8B949E', backdropFilter: 'blur(8px)' }}>
              📍 {cityName}
            </span>
          )}
          {weather?.temperature && (
            <span style={{ background: 'rgba(13,17,23,0.85)', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '3px 10px', fontSize: 11, color: '#8B949E', backdropFilter: 'blur(8px)' }}>
              {weatherIcon} {weather.temperature}°C
            </span>
          )}
          {trafficData && (
            <span style={{
              background: `rgba(${trafficData.overall==='macet'?'217,80,80':trafficData.overall==='padat'?'196,127,32':'31,173,142'},0.15)`,
              border: `0.5px solid rgba(${trafficData.overall==='macet'?'217,80,80':trafficData.overall==='padat'?'196,127,32':'31,173,142'},0.3)`,
              borderRadius: 20, padding: '3px 10px', fontSize: 11,
              color: trafficData.overall==='macet'?'#D95050':trafficData.overall==='padat'?'#C47F20':'#1FAD8E',
              backdropFilter: 'blur(8px)',
            }}>
              🚦 {trafficData.overall?.charAt(0).toUpperCase() + trafficData.overall?.slice(1)}
            </span>
          )}
          {/* Indikator GPS aktif */}
          {userLocation && (
            <span style={{ background: 'rgba(13,17,23,0.85)', border: '0.5px solid rgba(31,173,142,0.2)', borderRadius: 20, padding: '3px 10px', fontSize: 11, color: '#1FAD8E', backdropFilter: 'blur(8px)' }}>
              ● GPS Aktif
            </span>
          )}
        </div>
      ...
      {weather?.temperature && (
        <div
          style={{ position: 'absolute', top: cityName ? 102 : 66, right: 12, zIndex: 100, background: 'rgba(22,27,34,0.97)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '8px 12px', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.3)' }}

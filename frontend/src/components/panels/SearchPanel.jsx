...
export default function SearchPanel({ isOpen, onClose, onDestinationSelect, onPickLocation, mode }) {
  ...
  const handleSelect = (place) => {
    if (!isInIndonesia(place.lat, place.lng)) { toast.error('Lokasi di luar Indonesia tidak didukung.'); return; }
    if (onPickLocation) onPickLocation(place);
    else if (onDestinationSelect) onDestinationSelect(place);
    onClose();
  };
  ...
  return (
    <PanelBase isOpen={isOpen} onClose={onClose} title={mode === 'picker' ? 'Pilih Lokasi' : 'Cari Tujuan'}>
      {/* Input pencarian */}

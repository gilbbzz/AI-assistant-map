...
// FIX: AI sekarang memahami "dari X ke Y" dan menggunakan origin jika ada.
//      Error handling lebih baik, logging untuk debugging.
//      LocationPicker muncul setelah pencarian lokasi.
...
export default function AIChatPanel({ isOpen, onClose, onNavigateTo }) {
  ...
  // ── Fungsi pencarian tunggal ──────────────────────────────────────
  const searchPlace = useCallback(async (query, limit = 1) => {
    if (!query) return null;
    try {
      console.log('[AI] Mencari lokasi:', query);
      ...
      if (res.data?.data?.length > 0) {
        console.log(`[AI] Ditemukan ${res.data.data.length} hasil untuk "${query}"`);
        return limit === 1 ? res.data.data[0] : res.data.data;
      }
      console.warn('[AI] Tidak ada hasil untuk:', query);
      return null;
    } catch (err) {
      console.error('[AI] searchPlace error:', err.message);
      return null;
    }
  ...
  // ══════════════════════════════════════════════════════════════════
  // FUNGSI UTAMA: Cari lokasi dengan konfirmasi (DENGAN ORIGIN)
  // ══════════════════════════════════════════════════════════════════
  const searchAndConfirm = useCallback(async (aiData) => {
    console.log('[AI] searchAndConfirm dipanggil dengan data:', aiData);
    try {
      const { navigateTo, origin: originHint, locationType, locationCity, locationHint } = aiData;
      if (!navigateTo) {
        console.warn('[AI] navigateTo kosong, keluar.');
        return;
      }

      // 1. Cari ORIGIN jika ada hint
      let originPlace = null;
      if (originHint) {
        addAIMessage({
          role: 'assistant',
          ts: new Date(),
          content: `🔍 Mencari titik awal **"${originHint}"**...`,
        });
        originPlace = await searchPlace(originHint, 1);
        if (originPlace) {
          setOrigin(originPlace);
          addAIMessage({
            role: 'assistant',
            ts: new Date(),
            content: `✅ Titik awal ditemukan: **${originPlace.name}**`,
          });
        } else {
          addAIMessage({
            role: 'assistant',
            ts: new Date(),
            isError: true,
            content: `⚠️ Tidak dapat menemukan **"${originHint}"**. Saya akan menggunakan lokasi Anda sebagai titik awal.`,
          });
        }
      }

      // 2. Bangun query tujuan
      ...
      const candidates = await searchPlace(searchQuery, 5);
      if (!candidates || candidates.length === 0) {
        addAIMessage({
          role: 'assistant', ts: new Date(), isError: true,
          content: `Maaf, saya tidak dapat menemukan **"${navigateTo}"** di sekitar lokasi Anda. Coba sebutkan nama yang lebih spesifik atau tambahkan nama kota.`,
        });
        return;
      }
      ...
      if (filtered.length === 0) {
        addAIMessage({
          role: 'assistant', ts: new Date(), isError: true,
          content: `Maaf, tidak ada lokasi **"${navigateTo}"** yang valid di Indonesia.`,
        });
        return;
      }

      // Tampilkan LocationPicker
      addAIMessage({
        role:       'assistant',
        type:       'locationPicker',
        query:      navigateTo,
        ...
        onCancel:   () => {
          addAIMessage({
            role: 'assistant', ts: new Date(),
            content: `Oke, navigasi dibatalkan. Coba sebutkan nama atau alamat yang lebih spesifik.`,
          });
        },
      });
    } catch (err) {
      console.error('[AI] searchAndConfirm error:', err);
      addAIMessage({
        role: 'assistant', ts: new Date(), isError: true,
        content: `❌ Terjadi kesalahan: ${err.message || 'Coba lagi nanti.'}`,
      });
    }
  }, [userLocation, cityName, addAIMessage, searchPlace, setOrigin]);

  // ── Konfirmasi → buat rute ──────────────────────────────────────
  const handleNavigateConfirmed = useCallback((place) => {
    if (onNavigateTo) {
      console.log('[AI] Navigasi dikonfirmasi ke:', place.name);
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      console.log('[AI] Respons dari backend:', aiData);

      // Tampilkan reply teks
      addAIMessage({ role: 'assistant', content: reply, ts: new Date() });

      // Jika AI ingin navigasi → jalankan flow konfirmasi dengan origin
      if (aiData.navigateTo) {
        console.log('[AI] Memanggil searchAndConfirm dengan navigateTo:', aiData.navigateTo);
        await searchAndConfirm(aiData);
      } else {
        console.warn('[AI] Tidak ada navigateTo, hanya reply teks.');
      }
    } catch (err) {
